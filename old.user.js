...
function execute() {
	if (!isLoggedIn()) {
		if (DEBUG) { console.log("User not logged in. Please login and refresh the page."); }
		return;
	}
	const loggedInUsername = getLoggedInUsername();
	const chatListener = attachToChat();

	function attachToChat() {
		if (DEBUG) { console.log("Attaching to chat..."); }
        let currentChatCount = 0;

		return setInterval(function() {
            const chatList = findChatList();
            if (chatList == null) {
                if (DEBUG) { console.log("Could not find chat. Awaiting next iteration..."); }
                return;
            }

            //if (DEBUG) { console.log("Found chat."); }
            if (chatList.childElementCount > currentChatCount) {
                const newChatCount = chatList.childElementCount - currentChatCount;
                const newChatItems = [...chatList.children]
				                         .splice(newChatCount, chatList.children.length)
				                         .filter(x => x.className === "chat-line__message");

				//if (DEBUG) { console.log(newChatItems); }
				console.log("newChatItems", newChatItems);
				newChatItems.map(x => {
					const chatItem = [...x.children];
					const chatAuthor = chatItem.find(y => y.className === "chat-line__username").innerText;
					const chatMentions = chatItem.filter(y => [...y.classList.values()].includes("mention-fragment")).map(z => z.innerText);
					const chatMessage = chatItem.find(y => y.className === "text-fragment").innerText;

					if (chatMentions.includes(`@${loggedInUsername}`)) {
						const currentLocalDateTime = new Date().toLocaleString();
						console.log(`(${currentLocalDateTime}) ${chatAuthor}:${chatMessage}`);
