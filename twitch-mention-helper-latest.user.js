...
function addMentionIcon() {
	...
	addMentionIconEvents(mentionIconNode);
...
function addMentionIconEvents(mentionIconNode) {
	...
	const chatSettingsPopup = createNode(POPUP_CHAT_SETTINGS);
	const chatSettingsPopupCloseButtonNode = chatSettingsPopup.querySelector("button[data-test-selector=chat-settings-close-button-selector]");
	
	const removeChatSettingsPopupHandler = (event) => {
		if (!chatSettingsPopup.contains(event.target)
		|| mentionIconButtonNode.contains(event.target)
		|| chatSettingsPopupCloseButtonNode.contains(event.target)) {
			chatSettingsPopup.remove();
	...
	const insertChatSettingsPopupHandler = (event) => {
		...
		mentionIcon.after(chatSettingsPopup);
